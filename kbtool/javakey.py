...
import re
def processJavaSrc(src):
    src = re.sub(r'/\*[\S\s]+?\*/', r'', src)
    src = re.sub(r'(?m)^ +', r'', src)
    src = re.sub(r'public static final [A-z]+ ', '', src)
    return src

def t1():
    exec(processJavaSrc(rawJavaKeyDef))
    return locals()

JavaKeys = t1()
JavaKeys = {k: v if isinstance(v, int) else ord(v) for k,v in JavaKeys.items()}

def t2():
    exec(processJavaSrc(rawJavaMaskDef))
    return locals()
JavaModifiers = t2()

if __name__ == '__main__':
    print(JavaKeys, JavaModifiers)
